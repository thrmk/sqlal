...
def update_output_div(input_value,start,end):
    connection1 = engine#.connnect()
    print("start=",start,"end=",end,"dt.now=",datetime.today().strftime("%H:%M:%S %d/%m/%Y"))
    df=pd.read_sql("select * from smbtable",connection1)
    print("smb table=",df)
    #filtered_d = df[df.devId == selected_device]# and ([df.stamp == i] for i in (start,end,timedelta(microseconds=1)))]
    #filtered_df = df.loc[(datetime.strptime(df['stamp'].to_string(),"%H:%M:%S %d/%m/%Y") > datetime.strptime(start,"%H:%M:%S %d/%m/%Y")) & (df['stamp'].dt.strftime("%H:%M:%S %d/%m/%Y") <= datetime.strptime(end,"%H:%M:%S %d/%m/%Y"))]
    #rows = User.query.all()
    #print("table filtereddf=",filtered_df.stamp)
    #print("table filtereddf=",filtered_df.all)
#    for 
...
#    filtered_df=filtered_df.all
    #filtered_df=filtered_df.values.tolist()
    df=df.values.tolist()
    #print("smbtablefilter=",filtered_df)
...
def update_download_link(input_value,start,end):
    ...
    filtered_df = df.loc[(df['stamp'] > start) & (df['stamp'] <= end)]
    filtered_df=filtered_df.values.tolist()
    con_df = pd.DataFrame(filtered_df, columns=['id','stamp','devId','str1','str2','str3','str4','str5','str6','str7','str8','str9','str10','str11','str12','str13','vol1','vol2','vol3','vol4','vol5','vol6','vol7','vol8','vol9','vol10','vol11','vol12','vol13','temp'])
    print("con_df=",con_df)
